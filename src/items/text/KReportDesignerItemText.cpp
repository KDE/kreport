...
void KReportDesignerItemText::init(QGraphicsScene *scene)
{
    ...
    updateRenderText(controlSource->value().toString(), itemValue->value().toString(),
...
{
    return QFontMetrics(fontValue()).boundingRect(int (x()), int (y()), 0, 0, textFlags(), renderText());
}
...
void KReportDesignerItemText::buildXML(QDomDocument *doc, QDomElement *parent)
{
    ...
    entity.setAttribute(QLatin1String("report:bottom-padding"), bottomPadding);
